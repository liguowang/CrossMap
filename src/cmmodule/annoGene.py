...
def getExonFromFile(bedfile):
    '''
    Extract ALL exon regions from input bed file (must be 12-column).
    return list of [chrom:+ st end]
    '''
    ret_lst=[]
    for line in open(bedfile,'r'):
        try:
...
def getExonFromLine(bedline):
    '''
    Extract ALL exon regions from input bed line (must be 12-column).
    return list of [chrom st end]
    '''
    ret_lst = []
    line = bedline
...
def annotateBed(inputbed,refbed,outfile):
    ...
    ref_exon_ranges = {}
    ref_exon_starts = collections.defaultdict(set)
    ...
    for line in open(inputbed, 'r'):
        ...
        if key in ref_exon_ranges:
            if  len(ref_exon_ranges[key].find(tx_start,tx_end) )== 0:
