...
def crossmap_region_file(mapping, inbed,
                         outfile=None, min_ratio=0.85, cstyle='a'):
    '''
    ...
    for line in ireader.reader(inbed):
        ...
        try:
            ...
            print(
                "Start coordinate is not an integer. skip " + line,
                file=sys.stderr)
            if outfile:
        ...
        try:
            ...
            print(
                "End coordinate is not an integer. skip " + line,
                file=sys.stderr)
            if outfile:
        ...
        if int(fields[1]) > int(fields[2]):
            print(
                "\"Start\" is larger than \"End\" coordinate. skip " + line,
                file=sys.stderr)
            if outfile:
        ...
        if len(a) == 2:
            ...
            if outfile is None:
                ...
                print(
                    '\t'.join([str(i) for i in fields]) + "\tmap_ratio=1.0000",
                    file=FILE_OUT)

        # when a is an even number but bigger than 2, each segment is
        # 100% match, but the whole region is not. In this case, check
        # *min_ratio* of the query
        if len(a) > 2:
            # EVEN: [('chr1', 246974830, 246974833, '+'), ('chr1', 247024833, 247024835, '+')]
            a_query = a[::2]
            a_query_mapped_nt = sum([i[2]-i[1] for i in a_query])
            ...
            if map_ratio >= min_ratio:
                if len(a_target_chroms) == 1:
                    ...
                    if outfile is None:
                        print(line + '\tFail\tCrossChroms')
                    else:
                        print(line + '\tFail\tCrossChroms', file=UNMAP)
