...
def crossmap_bam_file(mapping, chainfile, infile, outfile_prefix,
                      chrom_size, IS_size=200, IS_std=30.0, fold=3,
                      addtag=True, cstyle='a'):
    ...
    fold : float
        ...
        different strand and the distance between them is less then \
        fold * stdev from the mean.
    ...
    if infile.lower().endswith('.bam'):
        ...
        print("Unknown file type! Input file must have suffix '.bam',\
              '.cram', or '.sam'.", file=sys.stderr)
        sys.exit(1)
    ...
    chrom_style = sam_ori_header['SQ'][0]['SN']

    # update chrom_size of target genome
    target_chrom_sizes = {}
    for n, l in chrom_size.items():
        cid = update_chromID(chrom_style, n, chr_style=cstyle)
        target_chrom_sizes[cid] = l
    ...
    if outfile_prefix is not None:
        if file_type == 'BAM':
            ...
            logging.info(
                "Liftover BAM file \"%s\" to \"%s\""
                % (infile, outfile_prefix + '.bam'))
        elif file_type == 'CRAM':
            OUT_FILE = pysam.Samfile(
                outfile_prefix + '.bam', "wb",
                header=new_header)
            logging.info("Liftover CRAM file \"%s\" to \"%s\""
                         % (infile, outfile_prefix + '.bam'))
        elif file_type == 'SAM':
            OUT_FILE = pysam.Samfile(
                outfile_prefix + '.sam', "wh",
                header=new_header)
            logging.info(
                "Liftover SAM file \"%s\" to \"%s\""
                % (infile, outfile_prefix + '.sam'))
        else:
            logging.error(
                "Unknown file type! Input file must have suffix '.bam',\
                '.cram', or '.sam'.")
            sys.exit(1)
    # write to screen
    else:
    ...
    try:
        while(1):
            ...
            # by default pysam will change RG:Z to RG:A, which can cause
            # downstream failures with GATK and freebayes
            try:
            ...
            if old_alignment.is_paired:
                # pair-end in sequencing
                new_alignment.flag = 0x1
                ...
                # ==================================
                    ...
                    # R1 unmap, R2 is mapped
                        ...
                        try:
                            ...
                            if old_alignment.mate_is_reverse:
                                read2_strand = '-'
                            else:
                                read2_strand = '+'
                            ...
                            read2_maps = map_coordinates(
                                ...
                                read2_strand, chrom_style=cstyle)
                        except:
                            read2_maps = None
                ...
                # ==================================
                    ...
                    try:
                        ...
                        read1_maps = map_coordinates(
                            mapping, read1_chr, read1_start, read1_end, read1_strand, chrom_style=cstyle)
                    except:
                        read1_maps = None
                    ...
                    if not old_alignment.mate_is_unmapped:
                        try:
                            ...
                            read2_end = read2_start + 1
                            read2_maps = map_coordinates(
                                mapping, read2_chr, read2_start, read2_end, read2_strand, chrom_style=cstyle)
                        except:
                            read2_maps = None
                    # ------------------------------------
    ...
    if outfile_prefix is not None:
        if file_type == "BAM" or file_type == "CRAM":
            try:
                logging.info(
                    'Sort "%s" and save as "%s"' % (outfile_prefix + '.bam', outfile_prefix + '.sorted.bam'))
                pysam.sort("-o",  outfile_prefix + '.sorted.bam', outfile_prefix + '.bam')
            except:
                logging.warning("output BAM file was NOT sorted")
            try:
                logging.info(
                    'Index "%s" ...' % (outfile_prefix + '.sorted.bam'))
                pysam.index(
                    outfile_prefix + '.sorted.bam',outfile_prefix + '.sorted.bam.bai')
            except:
                logging.warning(
                    "output BAM file was NOT indexed.")
